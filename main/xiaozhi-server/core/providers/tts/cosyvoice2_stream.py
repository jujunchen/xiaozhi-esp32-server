...
class TTSProvider(TTSProviderBase):
    def __init__(self, config, delete_audio_file):
        ...
        self.voice = config.get("voice", "湾湾小何")
    ...
    async def text_to_speak(self, text, is_last):
        ...
        payload = {"tts_text": text, "spk_id": self.voice, "stream": True}
    ...
    def to_tts(self, text: str) -> list:
        ...
        payload = {"tts_text": text, "spk_id": self.voice}

        try:
            with requests.post(self.api_url, json=payload, timeout=600) as response:
